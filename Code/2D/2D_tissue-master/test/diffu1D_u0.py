...
def test_solvers():
    ...
    def compare(u, x, t, n):      # user_action function
        ...
        diff = abs(u_e - u).max()
        tol = 1E-4
        print diff > tol, 'max diff: %g, %g' % (diff,t[n])

    import functools
    s = functools.partial  # object for calling a function w/args
    solvers = [
        s(solver_FE_simple, I=I, a=a, f=f, L=L, dt=dt, F=F, T=2),
        ...
        s(solver_BE_simple, I=I, a=a, f=f, L=L, dt=dt, F=F, T=2,
          user_action=compare),
        s(solver_BE,        I=I, a=a, f=f, L=L, dt=dt, F=F, T=5,
          user_action=compare),
    ...
    print u_e
    print u
    # print(3.33067e-16>tol)
    print diff < tol, 'max diff solver_FE_simple  1111: %g' % diff
