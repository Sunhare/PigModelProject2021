...
int main (int argc, char **argv) {
	...
	double * cell_para_vec = new double [600 * 18];

	memset(cell_para_vec, 0, 600 * 18 * sizeof(double));
	read_num_data_file("para_log.dat.New", cell_para_vec, 600 * 18);
	...
	*/
...

	{
		...
		for (int i = 0; i < count; ++i)
		{
			...
			// cell_vec[i]->cell.assign_cell_pop_para(&cell_para_vec[cell_vec[i]->m_cell_type_ID * 17]); //(i + top.starting_index)*17]);
