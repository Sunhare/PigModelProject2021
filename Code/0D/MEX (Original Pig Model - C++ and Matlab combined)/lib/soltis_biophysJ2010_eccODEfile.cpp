#ifndef _SOLTIS_BIOPHYS_
#define _SOLTIS_BIOPHYS_
...
// void soltis_biophysJ2010_eccODEfile(double t, double *y, double *ydot, double *p, Cell *cell) {
...
int soltis_biophysJ2010_eccODEfile(realtype t, N_Vector Y, N_Vector YDOT, void *user_data) {
	...
	const double* p = cell->get_p();
	const int NEQ = cell->get_NEQ();
	...
	bool AP_Clamp_Switch = (bool)p[1];

	double I_stim;
	if (AP_Clamp_Switch == true){
		I_stim = S1( 0, 0.001, p[0], t, 1); //Used to measure APD
	}
	else{
		if(p[8] > 0){ //S1S2 pacing
			...
			I_stim = S1S2(0, 22, p[0], p[2]-1, p[8], t, 3);
		}
		else{
			I_stim = S1( 0, 22, p[0], t, 3);	
		}
	}
	

	double VOLTAGE_VARIABLE = 0;
	double RESISTANCE_PARAMETER = 0.3;
	double I_command = 0;
	...
	if(AP_Clamp_Switch == true){
		...
		int tn = t/cell->dt;
		int pcln = p[0]/cell->dt;

		VOLTAGE_VARIABLE = cell->ap_clamp_data[tn%pcln];

		I_command = (VOLTAGE_VARIABLE- y[38])/RESISTANCE_PARAMETER;
	}

	//These are my models parameters (13 scaling factors)
	// int size_const_array = 8;
	int size_const_array = 9;

	cell->SCALE_ICa 	= 	p[size_const_array +  0];
	cell->SCALE_INa 	= 	p[size_const_array +  1];
	cell->SCALE_INaL 	= 	p[size_const_array +  2];
	cell->SCALE_IK1 	= 	p[size_const_array +  3];
	cell->SCALE_IKr 	= 	p[size_const_array +  4];
	cell->SCALE_IKs 	= 	p[size_const_array +  5];
	cell->SCALE_Isk 	= 	p[size_const_array +  6];
	cell->SCALE_IClCa 	= 	p[size_const_array +  7];
	cell->SCALE_INCX 	= 	p[size_const_array +  8];
	cell->SCALE_INaK 	= 	p[size_const_array +  9];
	cell->SCALE_JSRrel 	= 	p[size_const_array + 10];
	cell->SCALE_JSERCA 	= 	p[size_const_array + 11];
	cell->SCALE_Ito 	= 	p[size_const_array + 12];
	cell->SCALE_koCa 	= 	p[size_const_array + 13];

	//These are constants in my model so they are no longer parameters
	//  CaMKII phosphorylated targets (// )
	double LCC_CKp     = 0.547677438908217;		//1: LCC_CKp  p[1];
	double RyR_CKp     = 0.195250274557101;		//2: RyR_CKp p[2];
	double PLB_CKp     = 0.0120195262172836;	//3: PLB_CKp p[3];

	//  PKA phosphorylated targets (// )
	double LCCa_PKAp   = 0.0277913210987449;	//4: LCCa_PKAp p[4];
	double LCCb_PKAp   = 0.0327912283306637;	//5: LCCb_PKAp p[5];
	double PLB_PKAn    = 0.992681233585950;		//6: PLB_PKAn //  This is % non-phosphorylated PLB targe]s p[6];  //  This is % non-phosphorylated PLB targe]s
	double RyR_PKAp    = 0.0243941092361056;	//7: RyR_PKAp p[7];
	double TnI_PKAp    = 0.00312520780089506;	//8: TnI_PKAp p[8];
	double IKs_PKAp    = 0.0733327839312986;	//9: IKs_PKAp p[9];
	double ICFTR_PKAp  = 0.162146087312334;		//10: ICFTR_PKAp p[10];

	//  Flag for CKII OE (tells code whether or not it should use Ito and INa changes) - is either 0 (WT) or 1 (OE)
	double CKIIOE      = 0; // p[11];
	double rest 	   = 1; //p[12];

	
	

	// double ydot = zeros(size(y));
	// //  Model Parameters
	//  Constants
	...
	double Cmem = cell->cmem;   //  [F] membrane capacitance
	...
	double Cli = cell->cli;   //15  Intracellular Cl  [mM]
	double Clo = cell->clo;//150;  //  Extracellular Cl  [mM]
	double Ko = cell->ko;//5.4;   //  Extracellular K   [mM]
	double Nao = cell->nao;//140;  //  Extracellular Na  [mM]
	double Cao = cell->cao;//1.8;  //  Extracellular Ca  [mM] 
	double Mgi = cell->mgi;    //  Intracellular Mg  [mM]
	...
	pNa *= cell->SCALE_ICa; //Scaling Factor ICaL Na permeability
	pCa *= cell->SCALE_ICa; //Scaling Factor ICaL Ca permeability
	pK  *= cell->SCALE_ICa; //Scaling Factor ICaL K  permeability
	...
	koCa *= cell->SCALE_koCa;       //  [mM^-2 1/ms]
	...
	I_Na_junc *= cell->SCALE_INa; //Scaling Factor INa (fast) Junctional
	I_Na_sl *= cell->SCALE_INa; //Scaling Factor INa (fast) Sub Sarcolemmal
	...
	I_Nalj *= cell->SCALE_INaL; //Scaling Factor INaL Junctional
	I_Nalsl *= cell->SCALE_INaL; //Scaling Factor INaL Sub Sarcolemmal
	...
	I_nak_junc *= cell->SCALE_INaK; //Scaling Factor I_NaK Junctional
	I_nak_sl *= cell->SCALE_INaK; //Scaling Factor I_NaK Sub Sarcolemmal
	...
	I_kr *= cell->SCALE_IKr; //Scaling Factor I_Kr;
	...
	I_ks_junc *= cell->SCALE_IKs; //Scaling Factor I_ks Junctional
	I_ks_sl *= cell->SCALE_IKs; //Scaling Factor I_ks Sub Sarcolemmal

	double I_ks = I_ks_junc + I_ks_sl;

	// // I_sk: Small Conductance Calcium Sensitive Potassium Channel from Kennedy et al 2017
	// // TODO CODE NOW HAS COMPARTMENTALIZATION AND CORRECT UNITS
	...
	I_sk_junc *= cell->SCALE_Isk; //Scaling Factor I_sk Junctional
	I_sk_sl *= cell->SCALE_Isk; //Scaling Factor I_sk Sub Sarcolemmal
	...
	//Redundant, but set Scaling of I_to to 0
	I_tof *= cell->SCALE_Ito; //Scaling Factor I_tof
	I_tos *= cell->SCALE_Ito; //Scaling Factor I_tos
	...
	I_ki *= cell->SCALE_IK1; //Scaling Factor I_k1

	// //  I_ClCa: Ca-activated Cl Current, I_Clbk: background Cl Current

	// double KdClCa = 100e-3;    //  [mM]
	double KdClCa = 10e-3;    //  [mM]
	...
	I_ClCa *= cell->SCALE_IClCa; //Scaling Factor I_Clca
	...
	I_ncx_junc *= cell->SCALE_INCX; //Scaling Factor I_NCX Junctional
	I_ncx_sl *= cell->SCALE_INCX; //Scaling Factor I_NCX Sarcolemmal
	...
	//Unknown Ca related current parameters (probably for output, never used elsewhere)
	ydot[40] = 0;
	ydot[41] = 0;
	ydot[42] = 0;
	ydot[43] = 0;
	...
	ydot[38] = -( I_tot ) + I_stim + I_command; //Voltage from current, Stimulation, and AP Clamp respectively
	cell->dVdt = ydot[38]; //dV/dt

	//// Cell assignment
	cell->t = t;
	cell->Vm = y[38];//Voltage
	...
	//SR Parameters
	cell->caSR = y[30]; 
	cell->J_SRrel = J_SRCarel;
	cell->J_serca = J_serca;

	//Na Currents
	cell->I_Na = I_Na_fast; //Fast component of I_Na
	cell->I_NaL = I_Nal;
	...
	cell->I_Ks = I_ks; 
	cell->I_sk = I_sk;// Calcium dependent potassium channel
	...
	cell->I_SLCaP = I_pca;
	cell->I_CaBK = I_cabk;
	cell->J_SRLeak = J_SRleak;
	cell->I_NaBK = I_nabk;
	cell->I_ClBK = I_Clbk;

	cell->I_stim = I_stim;

	cell->caJ = y[35]; //Junctional Calcium
	cell->caSL = y[36]; //Sarcolemmal Calcium
	...
	// std::cout << "Ended soltis function" << std::endl;
	return 0;
}
