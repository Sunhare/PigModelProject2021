...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
	...
	if(thread_ID >= 0){
		...
		int n2 = sprintf (buffer_results, "outputs/results/%s_results_%.0f_s2_%d.txt", cell_name.c_str(), pcl, s2_pcl); //n is a placeholder. Write the string to buffer
	}
	else if(thread_ID == -2){ //EAD test
		std::cout << "EAD TEST" << std::endl;
		...
		switch(cell_type){
		case 1:
			n2 = sprintf(buffer_results,  "./outputs/results/%s_results_%.0f_%04.2f_%04.2f_%04.2f_.txt", cell_name.c_str(), pcl, p[n_const-1+1]/1.17182814104286, p[n_const-1+5]/1.34601288054242, p[n_const-1+6]/1.36651622380483);
			break;
		case 2:
			n2 = sprintf(buffer_results,  "./outputs/results/%s_results_%.0f_%04.2f_%04.2f_%04.2f_.txt", cell_name.c_str(), pcl, p[n_const-1+1]/0.80, p[n_const-1+5]/1.19, p[n_const-1+6]/2.20);
			break;
		case 3:
			n2 = sprintf(buffer_results,  "./outputs/results/%s_results_%.0f_%04.2f_%04.2f_%04.2f_.txt", cell_name.c_str(), pcl, p[n_const-1+1]/0.87, p[n_const-1+5]/1.20, p[n_const-1+6]/2.00);
			break;
		}
