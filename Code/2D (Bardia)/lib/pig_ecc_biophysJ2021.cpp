...
public:
	...
	void initialize_state_vars(void);

	void pig_ecc_biophysJ2021(double t);
...
void PIG_ECC::initialize_state_vars(void){
...
void PIG_ECC::pig_ecc_biophysJ2021(double t){
	...
	const double* p = pcell->get_p();
	const int NEQ = pcell->get_NEQ();
	...
	if(pcell->is_stim == true){
		I_stim = S1( 0, 22, PCL, t, 3);
	}
	else {
		I_stim = 0;	
	}
	...
	pcell->SCALE_ICa 	= 	p[size_const_array +  0];
	pcell->SCALE_INa 	= 	p[size_const_array +  1];
	pcell->SCALE_INaL 	= 	p[size_const_array +  2];
	pcell->SCALE_IK1 	= 	p[size_const_array +  3];
	pcell->SCALE_IKr 	= 	p[size_const_array +  4];
	pcell->SCALE_IKs 	= 	p[size_const_array +  5];
	pcell->SCALE_Isk 	= 	p[size_const_array +  6];
	pcell->SCALE_IClCa 	= 	p[size_const_array +  7];
	pcell->SCALE_INCX 	= 	p[size_const_array +  8];
	pcell->SCALE_INaK 	= 	p[size_const_array +  9];
	pcell->SCALE_JSRrel 	= 	p[size_const_array + 10];
	pcell->SCALE_JSERCA 	= 	p[size_const_array + 11];
	pcell->SCALE_Ito 	= 	p[size_const_array + 12]; //Always set to 0 (removed from model, not present in pigs)
	pcell->SCALE_koCa 	= 	p[size_const_array + 13];
	...
	double Cmem = pcell->cmem;   //  [F] membrane capacitance
	...
	double Cli = pcell->cli;   //15  Intracellular Cl  [mM]
	double Clo = pcell->clo;//150;  //  Extracellular Cl  [mM]
	double Ko = pcell->ko;//5.4;   //  Extracellular K   [mM]
	double Nao = pcell->nao;//140;  //  Extracellular Na  [mM]
	double Cao = pcell->cao;//1.8;  //  Extracellular Ca  [mM] 
	double Mgi = pcell->mgi;    //  Intracellular Mg  [mM]
	...
	pNa *= pcell->SCALE_ICa; //Scaling Factor ICaL Na permeability
	pCa *= pcell->SCALE_ICa; //Scaling Factor ICaL Ca permeability
	pK  *= pcell->SCALE_ICa; //Scaling Factor ICaL K  permeability
	...
	koCa *= pcell->SCALE_koCa;       //  [mM^-2 1/ms]
	...
	I_Na_junc *= pcell->SCALE_INa; //Scaling Factor INa (fast) Junctional
	I_Na_sl *= pcell->SCALE_INa; //Scaling Factor INa (fast) Sub Sarcolemmal
	...
	I_Nalj *= pcell->SCALE_INaL; //Scaling Factor INaL Junctional
	I_Nalsl *= pcell->SCALE_INaL; //Scaling Factor INaL Sub Sarcolemmal
	...
	I_nak_junc *= pcell->SCALE_INaK; //Scaling Factor I_NaK Junctional
	I_nak_sl *= pcell->SCALE_INaK; //Scaling Factor I_NaK Sub Sarcolemmal
	...
	I_kr *= pcell->SCALE_IKr; //Scaling Factor I_Kr;
	...
	I_ks_junc *= pcell->SCALE_IKs; //Scaling Factor I_ks Junctional
	I_ks_sl *= pcell->SCALE_IKs; //Scaling Factor I_ks Sub Sarcolemmal
	...
	I_sk_junc *= pcell->SCALE_Isk; //Scaling Factor I_sk Junctional
	I_sk_sl *= pcell->SCALE_Isk; //Scaling Factor I_sk Sub Sarcolemmal
	...
	// I_tof *= pcell->SCALE_Ito; //Scaling Factor I_tof
	// I_tos *= pcell->SCALE_Ito; //Scaling Factor I_tos
	...
	I_ki *= pcell->SCALE_IK1; //Scaling Factor I_k1
	...
	I_ClCa *= pcell->SCALE_IClCa; //Scaling Factor I_Clca
	...
	I_ncx_junc *= pcell->SCALE_INCX; //Scaling Factor I_NCX Junctional
	I_ncx_sl *= pcell->SCALE_INCX; //Scaling Factor I_NCX Sarcolemmal
	...
	pcell->dVdt = ydot[38]; //dV/dt

	//// PCell assignment for output
	pcell->t = t;
	pcell->Vm = y[38];//Voltage
	...
	//SR Parameters
	pcell->caSR = y[30]; 
	pcell->J_SRrel = J_SRCarel;
	pcell->J_serca = J_serca;

	//Na Currents
	pcell->I_Na = I_Na_fast; //Fast component of I_Na
	pcell->I_NaL = I_Nal; //Late sodium current
	...
	pcell->I_Ks = I_ks; 
	pcell->I_sk = I_sk;// Calcium dependent potassium channel
	...
	pcell->I_SLCaP = I_pca;
	pcell->I_CaBK = I_cabk;
	pcell->J_SRLeak = J_SRleak;
	pcell->I_NaBK = I_nabk;
	pcell->I_ClBK = I_Clbk;

	pcell->I_stim = I_stim;

	pcell->caJ = y[35]; //Junctional Calcium
	pcell->caSL = y[36]; //Sarcolemmal Calcium
