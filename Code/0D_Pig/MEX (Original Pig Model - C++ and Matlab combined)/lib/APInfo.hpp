...
public:
	...
	double CaSR_peak_out[3], CaSR_min_out[3];
	...
	//Bardia
	double v95, neg_dVdtmax, neg_dVdt_Time; 
	double APD95;
	double APD95_out[3];
	double neg_dVdtmax_out[3];
	double APD_last; 

	double plateau50;
	double plateau75;
	double plateau50_out[3];
	double plateau75_out[3];
	...
	Pig model measures:
		...
		Plateau50 (mV)
		Plateau75 (mV)
		Traingulation (APD95/APD50)
		Vrest (mV)
		Vmax (mV)
		dVdtmax (V/s)
		neg_dVdtmax (V/s)
		APD95_n-1 (ms)
		Calcium transient (peak time, amplitude, tau_CaT)
	...
	*/
	...
	void MeasurePigModel(double t, double Istim, double BCL, double dt, double Vm, double INa, double CaT, double dVdt, double CaSR);
	void MeasurePigModel_APClamp(double t, double Istim, double BCL, double dt, double Vm, double INa, double CaT, double dVdt);
	void ReportLastTwoPig();
...
//Make a different APD measuring method for AP Clamp
void APInfor::MeasurePigModel(double t, double Istim, double BCL, double dt, double Vm, double INa, double CaT, double dVdt, double CaSR) {
	...
	Pig model measures:
		...
		Plateau50 (mV)
		Plateau75 (mV)
		Traingulation (APD95/APD50), aka also APD50 (ms)
		Vrest (mV)
		Vmax (mV)
		dVdtmax (V/s)
		neg_dVdtmax (V/s)
	...
	*/
	...
	// If there's any Istim, start measurng
	if ( ((Istim >= 1e-3 || Istim <= -1e-3 ) && (fabs(Istim_prev) < 1e-10))) {


		// check if APD values have betweenn recorded or not.
		...
		APD_out_end[APD_out_swhich] = APD95;
		...
		neg_dVdtmax_out[APD_out_swhich] = neg_dVdtmax;
		...
		CaSR_peak_out[APD_out_swhich] = CaSR_peak;
		CaSR_min_out[APD_out_swhich] = CaSR_min;
		current_stim_counter[APD_out_swhich] = Stim_counter;

		plateau50_out[APD_out_swhich] = plateau50;
		plateau75_out[APD_out_swhich] = plateau75;
		...
		APD95 = -1.0; 
		CaSR_peak = -1;
		CaSR_min = std::numeric_limits<double>::infinity(); //infinity
	...
	*/
	...
	//After stimulation has just occurred
	if (CaD_switch == 1) {
	...
	*/
	...
	// std::cout << t-timeAPDstart << "\t" << APD_last << "\t" << APD_last*0.5 << "\t" << plateau50 <<std::endl;
	if (t-timeAPDstart > (APD_last*0.5)-1 and t-timeAPDstart < (APD_last*0.5)+1){
		plateau50 = Vm;
	}
	else if (t-timeAPDstart > (APD_last*0.75)-1 and t-timeAPDstart < (APD_last*0.75)+1){
		plateau75 = Vm;
	}
	...
	// std::cout << 'a' << std::endl;
	//So I need to measure APD95 here
	...
	else if ((APD_switch == 6) and (Vm_prev >= v90 and Vm <= v90)) {
		APD_switch = 0;
		APD90 = t - dVdt_Time;
		APD_switch = 7;
	...
	else if ((APD_switch == 7) and (Vm_prev >= v95 and Vm <= v95)){
		APD_switch = 0;
		APD95 = t - dVdt_Time;
		APD_last = APD95;
...
void APInfor::MeasurePigModel_APClamp(double t, double Istim, double BCL, double dt, double Vm, double INa, double CaT, double dVdt) {
	...
	*/
	...
	else if ((APD_switch == 5) and (Vm_prev >= v95 and Vm <= v95)){
		APD_switch = 0;
		APD95 = t - dVdt_Time;
		APD_last = APD95;
...
void APInfor::ReportLastTwoPig() {

	int n_width = 10;

	std::cout <<
	std::setw(n_width) << std::left << "APD95"                      <<
	std::setw(n_width) << std::left << "APD50"                      <<
	std::setw(n_width) << std::left << "Vmax"                       <<
	std::setw(n_width) << std::left << "Vmin"                       <<
	std::setw(n_width) << std::left << "dVdtmax"                    <<
	std::setw(n_width) << std::left << "-dVdtmax"                   <<
	std::setw(n_width) << std::left << "plat_avg"                   <<
	std::setw(n_width) << std::left << "plat50"                     <<
	std::setw(n_width) << std::left << "plat75"                     <<
	std::setw(n_width) << std::left << "INa_max"                    <<

	std::endl;

	for(int i = 0; i < 2; i++){
		std::cout <<
		std::setw(n_width) << std::left << APD_out_end[i]           <<
		std::setw(n_width) << std::left << APD50_out[i]             <<
		std::setw(n_width) << std::left << Vmax_out[i]              <<
		std::setw(n_width) << std::left << Vmin_out[i]              <<
		std::setw(n_width) << std::left << dVdtmax_out[i]           <<
		std::setw(n_width) << std::left << neg_dVdtmax_out[i]       <<
		std::setw(n_width) << std::left << plateau_potential_out[i] <<
		std::setw(n_width) << std::left << plateau50_out[i]         <<
		std::setw(n_width) << std::left << plateau75_out[i]         <<
		std::setw(n_width) << std::left << INa_max_out[i]           <<
		std::endl;
	}
}
