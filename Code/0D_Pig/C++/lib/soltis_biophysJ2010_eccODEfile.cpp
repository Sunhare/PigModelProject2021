...
int soltis_biophysJ2010_eccODEfile(realtype t, N_Vector Y, N_Vector YDOT, void *user_data) {
	...
	if (AP_Clamp_Switch == true){
		I_stim = S1( 0, 0.001, cycleLength, t, 1); //Used to measure APD
	}
	else if(cell->is_stim == true){
		if(s2_pcl > 0){
			...
			I_stim = S1S2(0, 22, cycleLength, nbeats-1, s2_pcl, t, 3);
		}
		else{
			I_stim = S1( 0, 22, cycleLength, t, 3);	
		}
	...
	cell->SCALE_Ito 	= 	p[size_const_array + 12]; //Always set to 0 (removed from model, not present in pigs)
	...
	cell->I_NaL = I_Nal; //Late sodium current
